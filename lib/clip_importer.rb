...
class ClipImporter
  def self.import_railscasts
    ...
    feed.entries.each do |entry|
      ...
      # Find or create the clip data into our database
      Clip.where(video_url: entry.video_url).first_or_create(
        title:        title,
    ...
    # Return the number of total clips for the source
    Clip.where(source: 'railscasts').count
  end
end